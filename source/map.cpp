...
bool Map::open(const std::string file)
{
	...
	for(int x = 20; ; x += 2) {
		int y = 22;
		Tile* old = getTile(x, y, bluerme::MapGroundLayer);
		if(old) {
			y -= 2;
			Tile* new_ = getTile(x, y, bluerme::MapGroundLayer);
			if(new_) {
...
bool Map::exportMinimap(FileName filename, int floor /*= bluerme::MapGroundLayer*/, bool displaydialog)
{
	uint8_t* pic = nullptr;
...
{
	if (uid < bluerme::MinUniqueId || uniqueIds.empty())
		return false;
