...
{
	texture = 0;
	buffer.resize(static_cast<size_t>(bluerme::ClientMapWidth * bluerme::ClientMapHeight * bluerme::PixelFormatRGBA));
...
{
	constexpr int half_tile_size = bluerme::TileSize / 2;

	for (int x = 0; x < bluerme::ClientMapWidth; ++x) {
		for (int y = 0; y < bluerme::ClientMapHeight; ++y) {
			int mx = (map_x + x);
			int my = (map_y + y);
			int px = (mx * bluerme::TileSize + half_tile_size);
			int py = (my * bluerme::TileSize + half_tile_size);
			int index = (y * bluerme::ClientMapWidth + x);
			int color_index = index * bluerme::PixelFormatRGBA;
	...
	const int draw_x = map_x * bluerme::TileSize - scroll_x;
	const int draw_y = map_y * bluerme::TileSize - scroll_y;
	constexpr int draw_width = bluerme::ClientMapWidth * bluerme::TileSize;
	constexpr int draw_height = bluerme::ClientMapHeight * bluerme::TileSize;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bluerme::ClientMapWidth, bluerme::ClientMapHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer.data());
...
{
	if (map_x <= 0 || map_x >= bluerme::MapMaxWidth || map_y <= 0 || map_y >= bluerme::MapMaxHeight) {
		return;
	}

	uint8_t intensity = std::min(light.intensity, static_cast<uint8_t>(bluerme::MaxLightIntensity));
