...
void MapCanvas::ScreenToMap(int screen_x, int screen_y, int* map_x, int* map_y)
{
	...
	if(screen_x < 0) {
		*map_x = (start_x + screen_x) / bluerme::TileSize;
	} else {
		*map_x = int(start_x + (screen_x * zoom)) / bluerme::TileSize;
	}

	if(screen_y < 0) {
		*map_y = (start_y + screen_y) / bluerme::TileSize;
	} else {
		*map_y = int(start_y + (screen_y * zoom)) / bluerme::TileSize;
	}

	if(floor <= bluerme::MapGroundLayer) {
		*map_x += bluerme::MapGroundLayer - floor;
		*map_y += bluerme::MapGroundLayer - floor;
	}/* else {
		*map_x += bluerme::MapMaxLayer - floor;
		*map_y += bluerme::MapMaxLayer - floor;
	}*/
}
					...
					switch(g_settings.getInteger(Config::SELECTION_TYPE)) {
						...
						case SELECT_ALL_FLOORS: {
							...
							if(g_settings.getInteger(Config::COMPENSATED_SELECT)) {
								start_x -= (floor < bluerme::MapGroundLayer ? bluerme::MapGroundLayer - floor : 0);
								start_y -= (floor < bluerme::MapGroundLayer ? bluerme::MapGroundLayer - floor : 0);

								end_x -= (floor < bluerme::MapGroundLayer ? bluerme::MapGroundLayer - floor : 0);
								end_y -= (floor < bluerme::MapGroundLayer ? bluerme::MapGroundLayer - floor : 0);
							}
			...
			switch(g_settings.getInteger(Config::SELECTION_TYPE)) {
				...
				case SELECT_ALL_FLOORS: {
					...
					for(int z = start_z; z >= end_z; z--) {
						...
						if(z <= bluerme::MapGroundLayer && g_settings.getInteger(Config::COMPENSATED_SELECT)) {
							start_x++; start_y++;
							end_x++; end_y++;
						}
					}
...
void MapCanvas::OnKeyDown(wxKeyEvent& event)
{
	...
	switch(event.GetKeyCode()) {
		...
		case WXK_UP: {
			...
			window->Scroll(start_x, int(start_y - bluerme::TileSize * tiles * zoom));
			UpdatePositionStatus();
			Refresh();
			break;
		}
		...
		case WXK_LEFT: {
			...
			window->Scroll(int(start_x - bluerme::TileSize * tiles * zoom), start_y);
			UpdatePositionStatus();
			Refresh();
			break;
		}
		...
		case '5': case '6': case '7': case '8': case '9': {
			...
			if(event.ControlDown()) {
				...
				if(g_gui.IsSelectionMode()) {
					...
					int view_start_map_x = view_start_x / bluerme::TileSize, view_start_map_y = view_start_y / bluerme::TileSize;

					int view_screensize_x, view_screensize_y;
					window->GetViewSize(&view_screensize_x, &view_screensize_y);

					int map_x = int(view_start_map_x + (view_screensize_x * zoom) / bluerme::TileSize / 2);
					int map_y = int(view_start_map_y + (view_screensize_y * zoom) / bluerme::TileSize / 2);
				...
				if(hk.IsPosition()) {
					...
					window->Scroll(bluerme::TileSize * map_x, bluerme::TileSize * map_y, true);
					floor = map_z;
...
{
	ASSERT(new_floor >= 0 || new_floor <= bluerme::MapMaxLayer);
	int old_floor = floor;
	floor = new_floor;
