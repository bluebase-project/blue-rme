...
void MapWindow::FitToMap()
{
	...
	SetSize(map.getWidth() * bluerme::TileSize, map.getHeight() * bluerme::TileSize, true);
}
...
void MapWindow::SetScreenCenterPosition(const Position& position, bool showIndicator)
{
	...
	int x = position.x * bluerme::TileSize;
	int y = position.y * bluerme::TileSize;
	int z = position.z;
	if(position.z < 8) {
		// Compensate for floor offset above ground
		x -= (bluerme::MapGroundLayer - z) * bluerme::TileSize;
		y -= (bluerme::MapGroundLayer - z) * bluerme::TileSize;
	}
