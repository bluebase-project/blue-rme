...
void OldPropertiesWindow::OnClickOK(wxCommandEvent& WXUNUSED(event))
{
	if(edit_item) {
		...
		if(!edit_item->getDepot()) {
			...
			if(uid_changed) {
				if(new_uid != 0 && (new_uid < bluerme::MinUniqueId || new_uid > bluerme::MaxUniqueId)) {
					wxString message = "Unique ID must be between %d and %d.";
					g_gui.PopupDialog(this, "Error", wxString::Format(message, bluerme::MinUniqueId, bluerme::MaxUniqueId), wxOK);
					return;
				}
			...
			if(aid_changed) {
				if(new_aid != 0 && (new_aid < bluerme::MinActionId || new_aid > bluerme::MaxActionId)) {
					wxString message = "Action ID must be between %d and %d.";
					g_gui.PopupDialog(this, "Error", wxString::Format(message, bluerme::MinActionId, bluerme::MaxActionId), wxOK);
					return;
				}
			}
		}
