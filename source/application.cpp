...
#include "../brushes/icon/bluerme_icon.xpm"
...
bool Application::OnInit()
{
	...
	std::cout << "Review COPYING in BLUERME distribution for details." << std::endl;
	...
	if(g_settings.getInteger(Config::ONLY_ONE_INSTANCE) && m_single_instance_checker->IsAnotherRunning()) {
		BLUERMEProcessClient client;
		wxConnectionBase* connection = client.MakeConnection("localhost", "bluerme_host", "bluerme_talk");
		if(connection) {
			wxString fileName;
	...
	m_proc_server = newd BLUERMEProcessServer();
	if(!m_proc_server->Create("bluerme_host")) {
    ...
    g_gui.root = newd MainFrame(__W_BLUERME_APPLICATION_NAME__, wxDefaultPosition, wxSize(700,500));
	...
	// Goto BLUERME website?
	if(g_settings.getInteger(Config::GOTO_WEBSITE_ON_BOOT) == 1) {
		::wxLaunchDefaultBrowser("http://www.github.com/bluesrc/blue-rme", wxBROWSER_NEW_WINDOW);
...
void Application::FixVersionDiscrapencies()
{
	...
	if(g_settings.getInteger(Config::VERSION_ID) < __BLUERME_VERSION_ID__ && ClientVersion::getLatestVersion() != nullptr){
	...
	wxFrame((wxFrame *)nullptr, -1, title, pos, size, wxDEFAULT_FRAME_STYLE)
{
	...
	SetStatusText(wxString("Welcome to ") << __W_BLUERME_APPLICATION_NAME__ << " " << __W_BLUERME_VERSION__);
...
bool MainFrame::DoQueryImportCreatures()
{
	if(g_creatures.hasMissing()) {
		long ret = g_gui.PopupDialog("Missing creatures", "There are missing creatures and/or NPC in the editor, do you want to load them from an OT pokemon/npc file?", wxYES | wxNO);
		if(ret == wxID_YES) {
			do {
