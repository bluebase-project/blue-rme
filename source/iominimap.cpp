...
bool IOMinimap::saveMinimap(const std::string& directory, const std::string& name, int floor)
{
	if(m_mode == MinimapExportMode::AllFloors || m_mode == MinimapExportMode::SelectedArea) {
		...
		if(floor < bluerme::MapMinLayer || floor > bluerme::MapMaxLayer) {
			floor = bluerme::MapGroundLayer;
		}
	}
...
bool IOMinimap::saveOtmm(const wxFileName& file)
{
	try
	{
		...
		for(uint8_t z = 0; z <= bluerme::MapMaxLayer; ++z) {
...
bool IOMinimap::exportMinimap(const std::string& directory)
{
	...
	wxRect bounds[bluerme::MapLayers];
	int min_z = m_floor == -1 ? 0 : m_floor;
	int max_z = m_floor == -1 ? bluerme::MapMaxLayer : m_floor;

	for (size_t z = min_z; z <= max_z; z++) {
		auto& rect = bounds[z];
		rect.x = bluerme::MapMaxWidth + 1;
		rect.y = bluerme::MapMaxHeight + 1;
		rect.width = 0;
		rect.height = 0;
	}
	...
	constexpr int pixels_size = image_size * image_size * bluerme::PixelFormatRGB;
	...
	for(size_t z = min_z; z <= max_z; z++) {
		...
		for (int h = 0; h < bluerme::MapMaxHeight; h += image_size) {
			for (int w = 0; w < bluerme::MapMaxWidth; w += image_size) {
...
{
	int min_x = bluerme::MapMaxWidth + 1;
	int min_y = bluerme::MapMaxHeight + 1;
	int min_z = bluerme::MapMaxLayer + 1;
	...
	int pixels_size = image_width * image_height * bluerme::PixelFormatRGB;
	uint8_t* pixels = new uint8_t[pixels_size];
