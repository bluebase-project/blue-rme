...
wxMemoryDC* GameSprite::getDC(SpriteSize size)
{
	...
	if(!dc[size]) {
		...
		int image_size = std::max<int>(width, height) * bluerme::SpritePixels;
		...
		for(uint8_t l = 0; l < layers; l++) {
			for(uint8_t w = 0; w < width; w++) {
				for(uint8_t h = 0; h < height; h++) {
					...
					if(data) {
						wxImage img(bluerme::SpritePixels, bluerme::SpritePixels, data);
						img.SetMaskColour(0xFF, 0x00, 0xFF);
						image.Paste(img, (width - w - 1) * bluerme::SpritePixels, (height - h - 1) * bluerme::SpritePixels);
						img.Destroy();
					}
				}
			}
		...
		if(size == SPRITE_SIZE_16x16 || image.GetWidth() > bluerme::SpritePixels || image.GetHeight() > bluerme::SpritePixels) {
...
void GameSprite::Image::createGLTexture(GLuint textureId)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bluerme::SpritePixels, bluerme::SpritePixels, 0, GL_RGBA, GL_UNSIGNED_BYTE, rgba);
...
uint8_t* GameSprite::NormalImage::getRGBData()
{
	...
	const int pixels_data_size = bluerme::SpritePixels * bluerme::SpritePixels * 3;
...
uint8_t* GameSprite::NormalImage::getRGBAData()
{
	...
	const int pixels_data_size = bluerme::SpritePixelsSize * 4;
	uint8_t* data = newd uint8_t[pixels_data_size];
	...
	while(read < size && write < pixels_data_size) {
		int transparent = dump[read] | dump[read + 1] << 8;
		if(use_alpha && transparent >= bluerme::SpritePixelsSize) // Corrupted sprite?
			break;
		read += 2;
		for(int i = 0; i < transparent && write < pixels_data_size; i++) {
...
void GameSprite::EditorImage::createGLTexture(GLuint textureId)
{
	...
	wxSize size(bluerme::SpritePixels, bluerme::SpritePixels);
	wxBitmap bitmap = wxArtProvider::GetBitmap(bitmapId, wxART_OTHER, size);

	wxNativePixelData data(bitmap);
	if(!data) return;

	const int imageSize = bluerme::SpritePixelsSize * 4;
	...
	for(size_t y = 0; y < bluerme::SpritePixels; ++y)
	{
		wxNativePixelData::Iterator row_start = it;

		for(size_t x = 0; x < bluerme::SpritePixels; ++x, it++)
		{
			uint8_t red = it.Red();
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, 0x812F); // GL_CLAMP_TO_EDGE
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, 0x812F); // GL_CLAMP_TO_EDGE
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bluerme::SpritePixels, bluerme::SpritePixels, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData);
...
uint8_t* GameSprite::TemplateImage::getRGBData()
{
	...
	for(int y = 0; y < bluerme::SpritePixels; ++y) {
		for(int x = 0; x < bluerme::SpritePixels; ++x) {
			uint8_t& red   = rgbdata[y*bluerme::SpritePixels*3 + x*3 + 0];
			uint8_t& green = rgbdata[y*bluerme::SpritePixels*3 + x*3 + 1];
			uint8_t& blue  = rgbdata[y*bluerme::SpritePixels*3 + x*3 + 2];

			uint8_t& tred   = template_rgbdata[y*bluerme::SpritePixels*3 + x*3 + 0];
			uint8_t& tgreen = template_rgbdata[y*bluerme::SpritePixels*3 + x*3 + 1];
			uint8_t& tblue  = template_rgbdata[y*bluerme::SpritePixels*3 + x*3 + 2];
...
uint8_t* GameSprite::TemplateImage::getRGBAData()
{
	...
	for(int y = 0; y < bluerme::SpritePixels; ++y) {
		for(int x = 0; x < bluerme::SpritePixels; ++x) {
			uint8_t& red   = rgbadata[y*bluerme::SpritePixels*4 + x*4 + 0];
			uint8_t& green = rgbadata[y*bluerme::SpritePixels*4 + x*4 + 1];
			uint8_t& blue  = rgbadata[y*bluerme::SpritePixels*4 + x*4 + 2];
