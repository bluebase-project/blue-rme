...
#ifdef __WINDOWS__
		dir.AppendDir("Blue Map Editor");
#else
		dir.AppendDir(".bluerme");
#endif
		dir.AppendDir("data");
		dir.Mkdir(0755, wxPATH_MKDIR_FULL);
		return dir.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR);
...
void GUI::ShowWelcomeDialog(const wxBitmap &icon) {
    ...
    welcomeDialog = newd WelcomeDialog(__W_BLUERME_APPLICATION_NAME__, "Version " + __W_BLUERME_VERSION__, FROM_DIP(root, wxSize(800, 480)), icon, recent_files);
...
void GUI::ChangeFloor(int new_floor)
{
	...
	if(tab) {
		...
		if(new_floor < bluerme::MapMinLayer || new_floor > bluerme::MapMaxLayer)
			return;
...
void GUI::FillDoodadPreviewBuffer()
{
	...
	if(GetBrushShape() == BRUSHSHAPE_SQUARE) {
		...
		if(GetBrushSize() == 1) {
			...
			area = int(0.5 + GetBrushSize() * GetBrushSize() * bluerme::PI);
		}
	}
