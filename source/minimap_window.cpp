...
void MinimapWindow::OnPaint(wxPaintEvent& event)
{
	...
	if(g_gui.IsRenderingEnabled()) {
		...
		if(g_settings.getInteger(Config::MINIMAP_VIEW_BOX)) {
			...
			int tile_size = int(bluerme::TileSize / canvas->GetZoom()); // after zoom

			int floor_offset = (floor > bluerme::MapGroundLayer ? 0 : (bluerme::MapGroundLayer - floor));

			view_start_x = view_scroll_x / bluerme::TileSize + floor_offset;
			view_start_y = view_scroll_y / bluerme::TileSize + floor_offset;
