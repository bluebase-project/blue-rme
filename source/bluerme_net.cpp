...
const wxEventType EVT_BLUERMENET_CONNECTION_ESTABLISHED = wxNewEventType();
const wxEventType EVT_BLUERMENET_CONNECTION_LOST = wxNewEventType();
const wxEventType EVT_BLUERMENET_SHOW_SERVLIST = wxNewEventType();


BLUERMENet::BLUERMENet(wxEvtHandler* event_dump) :
	event_dump(event_dump)
{
}

BLUERMENet::~BLUERMENet()
{
}

bool BLUERMENet::Connect()
{
	...
	wxEvtHandler::Connect(wxID_ANY, wxEVT_SOCKET, wxSocketEventHandler(BLUERMENet::HandleEvent));
...
void BLUERMENet::HandleEvent(wxSocketEvent& evt)
{
	...
	switch(evt.GetSocketEvent())
	{
		...
		{
			// Generate event
			wxCommandEvent event(EVT_BLUERMENET_CONNECTION_LOST);
			event.SetInt(0);
...
void BLUERMENet::OnParsePacket(NetworkMessage* nmsg)
{
...
void BLUERMENet::OnReceiveWelcome(NetworkMessage* nmsg)
{
	can_host = nmsg->ReadU8() == 1;

	wxCommandEvent event(EVT_BLUERMENET_CONNECTION_ESTABLISHED);
	event_dump->AddPendingEvent(event);
}

void BLUERMENet::OnReceiveDisconnect(NetworkMessage *nmsg)
{
	wxCommandEvent event(EVT_BLUERMENET_CONNECTION_LOST, wxID_ANY);
	event.SetString(wxstr(nmsg->ReadString()));
	event.SetInt(1);
	event_dump->AddPendingEvent(event);
}

void BLUERMENet::OnReceivePing(NetworkMessage *nmsg)
{
	// Send pong
