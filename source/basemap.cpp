		...
		for(; index < 16; ++index) {
			...
			if(QTreeNode* child = node->child[index]) {
				if(child->isLeaf) {
					...
					//printf("\t%p is leaf\n", child);
					for(it.local_z = 0; it.local_z < bluerme::MapLayers; ++it.local_z) {
...
MapIterator& MapIterator::operator++()
{
	...
	while(true) {
		...
		bool unwind = false;
		for(; index < bluerme::MapLayers; ++index) {
			//printf("\tChecking index %d of %p\n", index, node);
			if(QTreeNode* child = node->child[index]) {
				if(child->isLeaf) {
					QTreeNode* leaf = child;
					//printf("\t%p is leaf\n", child);
					for(; local_z < bluerme::MapLayers; ++local_z) {
						//printf("\t\tIterating over Z:%d of %p", local_z, child);
						if(Floor* floor = leaf->array[local_z]) {
							//printf("\n");
							for(; local_i < bluerme::MapLayers; ++local_i) {
							...
							if(local_i > bluerme::MapMaxLayer) {
